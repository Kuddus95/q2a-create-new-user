	...
	class q2a_create_new_user 
	{
		...
		function process_request($request)
		{
			...
			if(!empty($email_in))
			{
				...
				$userid_check = q2a_get_userid_by_email($email_in);
				if(empty($userid_check))
				{
					// does not exist yet 
					$handle_check = q2a_finduserhandle($handle_in);
					if(empty($handle_check))
					{
						...
						if(empty($validation_errors))
						{
							...
							$userid = q2a_create_new_user($email_in, $password_in, $handle_in, QA_USER_LEVEL_BASIC, true);
							$qa_content = qa_content_prepare();
							$qa_content['custom'] = '
							<div class="usercreated-wrap">
								<h3>
									A new user has been created with the informations below... 
								</h3>
								<p>
									Email: '.$email_in.'
								</p>
